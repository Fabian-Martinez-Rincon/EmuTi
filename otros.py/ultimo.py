...
class MyApp(tk.Frame):
    ...
    def automatic_operation(self):
        ...
        self.result_label.config(text="Operación automática realizada")
    ...
    def toggle_automatic(self):
        ...
        if new_state:
            self.auto_button.config(text="Automático ON", command=self.automatic_operation)
        else:
            self.auto_button.config(text="Automático OFF", command=lambda: None)  # No action when OFF
    ...
    def create_widgets(self):
        ...
        self.auto_button = tk.Button(self, text="Automático OFF", command=self.toggle_automatic)
        self.auto_button.grid(row=2, column=0, pady=10, padx=10, columnspan=2)
